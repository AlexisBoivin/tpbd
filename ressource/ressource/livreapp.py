...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def creerPersonnage(self):
        ...
        query1 = "INSERT INTO fiche_personnage (nom) VALUES (%(nom)s);"
        cursor = cnx.cursor(dictionary=True)
        cursor.execute(query1, parametre1)
        cursor.close()
        parametre2 = {
            "discipline1": self.comboBoxdiscipline1.currentText(),
            "discipline2": self.comboBoxdiscipline2.currentText(),
        ...
        query2 = """
			INSERT INTO disciplinesconnues (id_personnage, id_discipline) VALUES 
			(fiche_personnage.last_insert_id(), %(discipline1)s),
			(fiche_personnage.last_insert_id(), %(discipline2)s),
			(fiche_personnage.last_insert_id(), %(discipline3)s),
			(fiche_personnage.last_insert_id(), %(discipline4)s),
			(fiche_personnage.last_insert_id(), %(discipline5)s); """
        cursor = cnx.cursor(dictionary=True)
        cursor.execute(query2, parametre2)
        cursor.close()
        parametre3 = {
            "arme1": self.comboBoxArme1.currentText(),
            "arme2": self.comboBoxArme2.currentText()
        ...
        query3 = """
			...
			(fiche_personnage.last_insert_id(), %(arme1)s),
			(fiche_personnage.last_insert_id(), %(arme2)s);"""
        cursor = cnx.cursor(dictionary=True)
        cursor.execute(query3, parametre3)
    def supprimer_sauvegarde(self):
        parametre = {
            "id": self.spinBoxIdSauvegarde.value()
        }
        query = "DELETE FROM sauvegarde WHERE id = %(id)s"
        cursor = cnx.cursor(dictionary=True)
        cursor.execute(query, parametre)
        for (ligne) in cursor:
            self.comboBoxtitre.addItem(str(ligne["titre"]))
